...
def select_best_target_by_priority(targets, target_classes_priority):
    """
    Select the best target based on class priority and distance.
    
    Args:
        targets: List of target dictionaries with keys: 'dist', 'class', 'x1', 'y1', 'x2', 'y2', 'type', 'conf'
        target_classes_priority: List of class indices in priority order (index 0 = highest priority)
    
    Returns:
        best_target: The selected target dictionary, or None if no valid target found
    
    Algorithm:
        1. Iterate through target_classes_priority from highest to lowest priority
        2. For each priority class, find all targets of that class within FOV
        3. If targets exist for this class, select the closest one and return immediately
        4. If no targets for this class, continue to next priority class
        5. If no targets found for any priority class, return None
    """
    if not targets or not target_classes_priority:
        return None
    
    # Filter targets that are within FOV
    fov_targets = [t for t in targets if is_target_in_fov(t['x1'], t['y1'], t['x2'], t['y2'])]
    
    if not fov_targets:
        return None
    
    # Iterate through priority classes (highest priority first)
    for priority_class_idx in target_classes_priority:
        # Find all targets of this class
        class_targets = [t for t in fov_targets if t.get('class') == priority_class_idx or 
                        str(t.get('class', '')) == str(priority_class_idx)]
        
        if class_targets:
            # Select the closest target of this class
            best_target = min(class_targets, key=lambda t: t['dist'])
            return best_target
    
    # No targets found for any priority class
    return None
...
def detection_and_aim_loop():
    ...
    while _aimbot_running:
        try:
            image = frame_queue.get(timeout=0.005)
        except queue.Empty:
            time.sleep(0.001)
            continue
        except Exception as e:
        ...
        if all_targets and button_held:
            # Get target class priority list from config
            target_classes_priority = getattr(config, 'target_classes', [])
            
            # Use priority-based selection if target_classes is configured
            if target_classes_priority:
                # Use new priority-based target selection
                best_target = select_best_target_by_priority(all_targets, target_classes_priority)
            else:
                # Fallback to legacy behavior when no priority classes configured
